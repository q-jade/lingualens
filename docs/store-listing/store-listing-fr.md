...
Local
• Ollama (préréglage par défaut) — http://localhost:11434
• LM Studio — Utilise l'API locale native de LM Studio

Préréglages cloud
• OpenAI — api.openai.com
• DeepSeek — api.deepseek.com
• DeepL — Point de terminaison API gratuit ou Pro
...
Panneau latéral (Chrome 114+)
Ouvrez un espace de travail de traduction dédié depuis la popup. Définissez les langues source et cible, échangez-les, consultez les résultats et parcourez l'historique des traductions récentes stocké localement sur votre appareil.

Paramètres et intégration
Lors de la première installation, les Paramètres s'ouvrent automatiquement avec un court guide de configuration. Configurez les langues, le fournisseur par défaut, l'ordre de secours, les modèles de prompt LLM (de base et supplémentaire) et les options par fournisseur telles que la désactivation de la sortie « thinking » sur les modèles compatibles pour des traductions plus rapides et plus propres.
...
Si les raccourcis entrent en conflit avec d'autres extensions, réassignez les raccourcis LinguaLens dans chrome://extensions/shortcuts.
...
Astuce Ollama : exécutez `ollama pull llama3` et démarrez Ollama avec OLLAMA_ORIGINS="chrome-extension://*" si le navigateur ne peut pas se connecter.
