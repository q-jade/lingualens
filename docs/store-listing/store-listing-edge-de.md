...
Lokal
• Ollama (Standardvoreinstellung) — läuft auf localhost:11434
• LM Studio — Verwendet die native lokale API von LM Studio

Cloud-Voreinstellungen
• OpenAI, DeepSeek, DeepL (Kostenlos & Pro), Google Cloud Translation
...
Wenn Tastenkürzel mit anderen Erweiterungen kollidieren, weisen Sie die Kürzel in den Erweiterungs-Tastenkürzel-Einstellungen Ihres Browsers neu zu.
...
Ollama-Tipp: Führen Sie `ollama pull llama3` aus und starten Sie Ollama mit der Umgebungsvariable OLLAMA_ORIGINS gesetzt auf Ihre Erweiterungsherkunft, falls der Browser keine lokale Verbindung herstellen kann.
