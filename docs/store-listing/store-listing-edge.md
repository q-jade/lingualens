...
Local
• Ollama (default preset) — runs on localhost:11434
• LM Studio — Uses LM Studio's native local API

Cloud Presets
• OpenAI, DeepSeek, DeepL (Free & Pro), Google Cloud Translation
...
If shortcuts conflict with other extensions, reassign them in your browser's extension shortcut settings.
...
Ollama tip: run `ollama pull llama3` and start Ollama with `OLLAMA_ORIGINS` set to your extension origin if the browser cannot connect locally.
